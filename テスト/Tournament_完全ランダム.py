...
def _allocate_leaf_slots_random(
	participants: list[Participant],
	T: int,
	bye_pos: set[int],
	rnd,
...
def make_first_round_pairs_quarter_even(
	...
	if not avoid_same_group:
		rnd = random.Random(seed)
		slots = _allocate_leaf_slots_random(participants, T, set(bye_pos), rnd)
		return [(slots[2*i], slots[2*i+1]) for i in range(M)]
...
# ブラケット構築（決勝まで）
# --------------------
def build_full_bracket(participants: List[Participant], seed: Optional[int] = None, avoid_same_group: bool = True) -> List[Round]:
	restarts = 1000 # if len(participants) < 32 else 5000
	first_pairs = make_first_round_pairs_quarter_even(
		participants, seed=seed, restarts=restarts, avoid_same_group=avoid_same_group
	)
...
if __name__ == "__main__":
	...
	rounds = build_full_bracket(sample, seed=None, avoid_same_group=False)
